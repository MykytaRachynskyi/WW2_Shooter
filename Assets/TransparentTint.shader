...
Shader "FX/Glass/Sublte" {
	...
	_BumpAmt("Distortion", range(0,128)) = 10
		//        _TintAmt("TintAmount", range(0.4,1)) = 0.6
		_TintAmt("TintAmount", Color) = (1, 0, 0, 1)
...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : COLOR
	{
		...
		half4 tint = tex2D(_MainTex, i.uvmain) * _TintAmt;    // Multiply with tint color
															  //        half4 high = tex2D(_ScndTex, i.uvscnd);
															  // Adding the higlight at 20% strentgh then doing the darkening color effect
		return lerp(col, col * tint, _TintAmt.a);    // Apply tint amount based on tint alpha
	}
		ENDCG
	}
	}
